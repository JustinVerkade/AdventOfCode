...
void computer_copy(OptComputer_t* computer, int64_t* commands, int64_t commands_count)
{
    memcpy(commands, computer->commands, sizeof(int64_t) * commands_count);
}

void computer_memdiff(OptComputer_t* computer, int64_t* commands, int64_t commands_count)
{
    for (int64_t i=0; i<commands_count; i++) {
        int64_t val_0 = computer->commands[i];
        int64_t val_1 = commands[i];
        if (val_0 != val_1) {
            printf("Code: %ld <> %ld -> %ld\n", i, val_1, val_0);
        }
    }
}
...
{
    while (computer->program_counter >= 0 && computer->program_counter < 4000) {
        ...
        if (!mode_a && variable_index_a < 4000 && variable_index_a >= 0) {
        ...
        if (!mode_b && variable_index_b < 4000 && variable_index_b >= 0) {
        ...
        if (!mode_c && variable_index_c < 4000 && variable_index_c >= 0) {
    ...
    for (int64_t y=min_y; y<=max_y; y++) {
        for (int64_t x=min_x; x<=max_x; x++) {
            ...
            if (tile == NULL) {
                ...
                char color = 0x00;
                color = tile->type == 0 ? ' ' : '0';
                color = tile->type == 1 ? '#' : color;
                color = tile->type == 2 ? '@' : color;
                color = tile->type == 3 ? '_' : color;
                printf("%c", color);
            }
        }
        printf("\n");
...
int solve_puzzle(char* file_name)
{
    ...
    while (1) {
        ...
        if (computer->input_wait) {
            ...
            // computer_memdiff(computer, commands, commands_count);
            computer_copy(computer, commands, commands_count);

            int64_t ball_position_x = commands[388];
            int64_t pandle_position = commands[392];

            int64_t joystick = ball_position_x - pandle_position;
            joystick = joystick == 0 ? 0 : joystick > 0 ? 1 : -1;
            computer_write(computer, joystick);
            continue;
