...
static int32_t get_antinode_count(Antenna* antennas, int32_t antenna_count, int32_t map_size)
{
    ...
    for (int32_t i=0; i<antenna_count; i++) {
        for (int32_t j=0; j<antenna_count; j++) {
            ...
            {   // loop over offset positions to create anti nodes in positive direction
                int32_t antinode_position_x = antenna_2.position_x;
                int32_t antinode_position_y = antenna_2.position_y;
                while (in_map(antinode_position_x, antinode_position_y, map_size)) {
                    ...
                    antinode_position_x += offset_x;
                    antinode_position_y += offset_y;
                }
            }
            {   // loop over offset positions to create anti nodes in positive direction
                int32_t antinode_position_x = antenna_1.position_x;
                int32_t antinode_position_y = antenna_1.position_y;
                while (in_map(antinode_position_x, antinode_position_y, map_size)) {
                    antinodes[antinode_count].id = '#';
                    antinodes[antinode_count].position_x = antinode_position_x;
                    antinodes[antinode_count].position_y = antinode_position_y;
