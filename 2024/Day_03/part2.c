...
static int32_t solve_puzzle_input(char* input_data, int32_t input_length)
{
    ...
    for (int32_t i=0; i<input_length; i++) {
        ...
        if ((start_character != 'm') && (start_character != 'd'))
            continue;
        if ((start_character == 'm') && (enabled == 1)) {
            sum_of_mul += check_mul(start_pointer);
        } else if (start_character == 'd') {
            if (memcmp(start_pointer, "do()", 4) == 0) {
                enabled = 1;
            } else if (memcmp(start_pointer, "don't()", 7) == 0) {
                enabled = 0;
            }
