...
static int32_t process_steps(char* p_map, int32_t width, int32_t height, Robot* robots, int32_t robots_count)
{
    int32_t bytes = (width + 1) * height;
    char* p_map_copy = (char*)malloc(bytes);
    if (p_map_copy == NULL) {
    ...
    int32_t value = 0;
    int32_t seconds = 20000;
    int32_t lowest_second = 0;
    int32_t lowest_safety = 0x0FFFFFFF;
    for (int32_t i=1; i<seconds; i++) {
        do_step(width, height, robots, robots_count);
        int32_t safety_factor = calc_safety_factor(width, height, robots, robots_count);
        if (safety_factor < lowest_safety) {
            lowest_safety = safety_factor;
            lowest_second = i;
        }
    }
    free(p_map_copy);
...
int solve_puzzle(char* file_name)
{
    ...
    int32_t answer = process_steps(p_map, map_width, map_height, robots, robots_count);
