...
int32_t play_game(char* file_name, int32_t elf_power)
{
    ...
    if (result % 2 == 0 || total_elfs != elf_count) {
        return 0;
    }
    return answer;
}
...
{
    for (int32_t power=4; power<1000; power++) {
        int32_t result = play_game(file_name, power);
        if (result) {
            printf("Answer: %d\n", result);
            break;
        }
