...
void carts_print(Cart_t* carts, int32_t cart_count)
{
    for (int32_t i=0; i<cart_count; i++) {
        Cart_t* cart = &carts[i];
        printf("Cart %3d: X:%d, Y:%d, D:%d\n", i, cart->position.x, cart->position.y, cart->crashed);
    }
}
...
int solve_puzzle(char* file_name)
{
    ...
    for (int32_t i=0; i<1000000; i++) {
        ...
        int32_t last_cart_x = 0;
        int32_t last_cart_y = 0;
        int32_t valid_carts = 0;
        for (int32_t i=0; i<cart_count; i++) {
            Cart_t* cart = &carts[i];
            if (cart->crashed) {
                continue;
            }
            last_cart_x = cart->position.x;
            last_cart_y = cart->position.y;
            valid_carts++;
        }
        
        if (valid_carts == 1) {
            printf("Answer: %d,%d\n", last_cart_x, last_cart_y);
            break;
        }
    }
