...
Node_t* values_insert(Node_t* values, int32_t value_size, int32_t* value_count, Node_t* current, int32_t value)
{
...
int32_t values_pop(Node_t* current)
{
...
void values_print(Node_t* values, int32_t value_count)
{
...
{
    const int32_t players = 446;
    const int32_t last_marble = 71522;

    int32_t* scores = (int32_t*)calloc(players, sizeof(int32_t));
    if (scores == NULL) {
        printf("Failed to allocate memory for scores!\n");
        return 1;
    }

    int32_t value_size = 100000;
    ...
    int32_t player_turn = 0;

    for (int32_t marble=1; marble<=last_marble; marble++) {
        if (marble % 23) {
