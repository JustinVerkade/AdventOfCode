...
int32_t recursive(Node_t* node, Node_t *nodes, int32_t node_count)
{
    ...
    int32_t weights[2000];
    int32_t weights_count = sizeof(weights) / sizeof(int32_t);
    for(uint8_t tree_node=0; tree_node<node->tag_count; tree_node++)
    {
        ...
        for(uint32_t node=0; node<node_count; node++)
        {
            ...
            weights[tree_node] = recursive(node_ptr, nodes, node_count);
            weight_sum += weights[tree_node];
        }
    }

    int32_t sample_weight = weights[0];
    for(uint8_t tree_node=1; tree_node<node->tag_count; tree_node++)
    {
        if(sample_weight != weights[tree_node])
        {
            printf("Tag: Inbalance: %s\n", node->tag);

            for(uint8_t inbalance_node=0; inbalance_node<node->tag_count; inbalance_node++)
            {
                printf("\tWeight: %s, %d\n", node->nodes[inbalance_node], weights[inbalance_node]);
            }
            break;
        }
    }
...
int8_t findInbalance(Node_t* node, Node_t *nodes, int32_t node_count)
{
