...
void layers_regress(Layer_t* layers, int32_t layer_count)
{
    for (int32_t i=0; i<layer_count; i++) {
        layers[i].position -= layers[i].direction;
        if (layers[i].position == layers[i].dept) {
...
void layers_reset(Layer_t* layers, int32_t layer_count)
{
    for (int32_t i=0; i<layer_count-1; i++) {
        layers_regress(layers, layer_count);
    }
}
...
int32_t layers_test(int64_t start_pico, Layer_t* layers, int32_t layer_count)
{
    int32_t severity = 0;
    for (int32_t i=0; i<layer_count; i++) {
        Layer_t* layer = &layers[i];
        if (layer->position == 0) {
            severity += i * layer->dept + 1;
        }
...
int solve_puzzle(char* file_name)
{
    ...
    for (;;) {
        if ((picoseconds % 100) == 0) {
            printf("\rTesting: %ld", picoseconds);
            fflush(stdout);
        }
        int32_t severity = layers_test(picoseconds, layers, layer_count);
        if (severity == 0) {
            break;
        }
        picoseconds++;
